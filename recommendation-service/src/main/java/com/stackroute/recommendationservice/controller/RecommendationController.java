...
public class RecommendationController {
    ...
    public ResponseEntity<?> getProductRecommendationByCityAndCategory(@RequestParam("state") String state, @RequestParam("category") String category) {
        log.debug("state:category" + state + category);
        try {
            HashSet<IncomingProductData> recommendations = this.recommendationService.getProductRecommendationByCityAndCategory(state, category);
            return new ResponseEntity<>(recommendations, HttpStatus.OK);
        } catch (ProductNotFoundException e) {
            return new ResponseEntity("Product Not Exits", HttpStatus.CONFLICT);
        }
