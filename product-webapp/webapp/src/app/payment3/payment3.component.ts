...
export class Payment3Component implements OnInit {
  ...
  constructor(private _productdetailsService: ProductDetailsService,public dialogRef: MatDialogRef<Payment3Component>,private userservice:UpdateDetailsService,private router: Router,private tranactionservice:TransactionService,private emailservice:EmailService) { 
...
recipent:any;
  ngOnInit(): void {
    this._productdetailsService.getProductDetailsByEmail("raryasgg@gmail.com").subscribe((data:any)=>{
    ...
    this.userservice.getUserCoinByEmail("raryasgg@gmail.com").subscribe(data => {
  ...
  onClickSubmitForm() {
    ...
    this._productdetailsService.updateProductNotAvailable(this.updateForm.value.pid).subscribe(data =>
      console.log(data)
    )
    Swal.fire({ icon: 'success', title: 'Set for NotAvailable!!', text: 'Thank You!', })


console.log(this.coindata.email)
console.log(this.productdata.pemail)
console.log(this.productdata.pcoin)
    this.userservice.exchange(this.coindata.email,this.productdata.pemail,this.productdata.pcoin).subscribe(data =>
      console.log(data)
    )
    Swal.fire({ icon: 'success', title: 'Requested for Exchange!!', text: 'Thank You!', })
    ...
    this.tranactionservice.saveTransaction( this.transactionObj).subscribe(data =>
      console.log(data)
    )
    Swal.fire({ icon: 'success', title: 'Tranaction Saved!!', text: 'Thank You!', })
...
this.emailObj.recipient=this.coindata.email
    ...
    Swal.fire({ icon: 'success', title: 'Transaction Sucessfully!!', text: 'Thank You!', })
